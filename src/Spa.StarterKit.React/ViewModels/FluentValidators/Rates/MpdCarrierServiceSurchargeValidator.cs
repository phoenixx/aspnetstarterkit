...
{
    public class MpdCarrierServiceSurchargeValidator<T> : AbstractValidator<T> where T : MpdCarrierServiceSurchargeViewModel
    {
        private const int DAY_OF_WEEK_UNIT_ID = 7;
        private const int NOT_APPLICABLE_DIMENSIONS_ID = 1;

        public MpdCarrierServiceSurchargeValidator()
        {
            //Common
            ...
            //Model
            RuleFor(x => x.MpdCarrierServiceSurchargeModel).SetValidator(new MpdCarrierServiceSurchargeModelValidator());
            ...
            //Fuel surcharges
            When(x => x.MpdCarrierServiceSurchargeModel.Type == SurchargeModelType.Fuel, () =>
                {
                    RuleFor(x => x.Amount).InclusiveBetween(0, 100).WithMessage("Fuel surcharge must be between 0 and 100%");
                });

            //Days of week
            When(x => x.MpdCarrierServiceSurchargeModel.Type == SurchargeModelType.SpecificDayCollection 
                    || x.MpdCarrierServiceSurchargeModel.Type== SurchargeModelType.SpecificDayDelivery, () =>
                {
            ...
            //Consignment value
            When(x => x.MpdCarrierServiceSurchargeModel.Type == SurchargeModelType.ConsignmentValue, () =>
                {
    ...
    public class CarrierServiceSurchargeValidator<T> : AbstractValidator<T> where T : CarrierServiceSurchargeViewModel
    {
        private const int DAY_OF_WEEK_UNIT_ID = 7;
        private const int NOT_APPLICABLE_DIMENSIONS_ID = 1;
        ...
        public CarrierServiceSurchargeValidator()
        {
            ...
            //Model
            RuleFor(x => x.CarrierServiceSurchargeModel).SetValidator(new CarrierServiceSurchargeModelValidator());
